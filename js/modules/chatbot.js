async function callChatApi(elements, state, messageText) {
    ...
    try {
        const response = await fetch('URL_DE_TU_API/api/v1/chat/', {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'X-API-Key': 'TU_API_KEY', 'X-Application-ID': 'TU_APP_ID' },
            body: JSON.stringify({ message: messageText, session_id: state.chatSessionId, is_authenticated_user: false })
        });
...
export function setupChat(elements, state) {
    elements.chatForm.addEventListener('submit', async (event) => {
        event.preventDefault(); const userInput = elements.chatInput.value.trim(); if (!userInput) return;
        const userMessage = document.createElement('div'); userMessage.className = 'chat-message user'; userMessage.textContent = userInput;
        elements.chatMessages.appendChild(userMessage); elements.chatInput.value = ''; elements.chatMessages.scrollTop = elements.chatMessages.scrollHeight;
